...
// SoA arrays definition
std::vector<std::string> g_complaintTypeLower;
...
bool ServiceRequest::fromFields(const std::vector<std::string>& f) {
    ...
    agencyName               = f[4];
    complaintType            = f[5];
    complaintTypeLower       = complaintType;
    std::transform(complaintTypeLower.begin(), complaintTypeLower.end(), complaintTypeLower.begin(),
                   [](unsigned char c) { return std::tolower(c); });
    // Populate SoA arrays
    g_complaintTypeLower.push_back(complaintTypeLower);
    g_latitude.push_back(latitude);
    g_longitude.push_back(longitude);
    descriptor               = f[6];
